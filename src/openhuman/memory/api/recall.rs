...
//! [`crate::openhuman::memory::api::types`], so every historical `types::RecallOpts` path — including
//! the engine crate's `crate::openhuman::memory::engine::types::` alias — keeps resolving.
...
pub struct RecallOpts<'a> {
    /// Restrict recall to this namespace; `None` falls back to [`crate::openhuman::memory::api::types::GLOBAL_NAMESPACE`].
