...
//!
//! These are supertraits of [`crate::openhuman::memory::api::provider::MemoryProvider`], which is what
...
//! assumed. See [`crate::openhuman::memory::api::provider::MemoryProvider`] for that half.
//!
...
//! [`crate::openhuman::memory::api::traits::Memory`] trait keeps `anyhow::Result` — it is an internal
...
pub trait MemoryCore: Send + Sync {
    ...
    ///
    /// Unlike the engine's [`crate::openhuman::memory::api::traits::Memory`], which has a `store` and a
...
pub trait MemoryPortability: Send + Sync {
    ...
    ///
    /// Records carry their own [`crate::openhuman::memory::api::types::MemoryTaint`]; an importing
